...
The output bit should be reset (to 0) at the positive clock edge when reset is high. If both of the above events occur at the same time, 
reset has precedence. In the last 4 cycles of the example waveform below, the 'reset' event occurs one cycle earlier than the 'set' event, 
...
module top_module (
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin
            out[i] <= 0;
        end else if ( ~in[i] & in_delay[i]) begin
            out[i] <= 1'b1;
            end
    end
    end
